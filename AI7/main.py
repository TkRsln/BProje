# -*- coding: utf-8 -*-
"""
Created on Wed May 24 15:54:41 2023
...
from moduls_ import Generator as Generator
from moduls_ import Discriminator as Discriminator
...
#Models
generator = Generator(num_conditions=6,latent_dim=100)
discriminator = Discriminator(num_conditions=7)
...
for epoch in range(num_epochs):
    for i,(input_img,input_txt,output_img) in enumerate(dataloader):
        ...
        ## REAL-Value & Loss
        real_validity = discriminator(output_img,input_img,input_txt)
        #print(f'real_val:{real_validity.shape} / real_labels:{real_labels.shape}')
        real_loss = adversarial_loss(real_validity, real_labels)
        
        ## FAKE-Value & Loss
        fake_images = generator(input_img,input_txt)
        o_img = fake_images
        fake_validity = discriminator(fake_images,input_img,input_txt)
